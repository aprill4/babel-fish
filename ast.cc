...
inline Type *check_sys_type(SysType type, Context&context) {
	return type == SysType::INT ? context.Int32Type
...
void VarDeclare::generate(IRBuilder *irBuilder) {
	...
	if(value_ != nullptr) {
		if(value_->type_ == SysType::INT)
			constant = new ConstantInt(irBuilder->getModule(), 
	...
	if(irBuilder->getScope()->parent == nullptr)
		GlobalVariable::Create(irBuilder->getContext(), 
													 type_ == SysType::INT ? (irBuilder->getContext()).Int32Type : (irBuilder->getContext()).FloatType,
								 					 identifier_->id_,
													 isConst_,
													 constant,
													 irBuilder->getModule());
...
void FunctionDefinition::generate(IRBuilder *irBuilder) {
	...
	for(auto arg : formalArgs_->list_)
		if((arg->identifier_->dimension).size()) {
			...
			else total_element = -1;
			argsType.emplace_back(ArrayType::get(irBuilder->getContext(), 
																					 check_sys_type(arg->type_, irBuilder->getContext()),
																					 total_element));
		}
		else
			argsType.emplace_back(check_sys_type(arg->type_, irBuilder->getContext()));

	Function::Create(irBuilder->getContext(),
									 FunctionType::get(check_sys_type(resultType_, irBuilder->getContext()),argsType),
									 {},
									 identifier->id_,
									 irBuilder->getModule());
