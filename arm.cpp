...
void scan_order(MachineBasicBlock *mb, std::set<MachineBasicBlock*>&visited) {
    for(auto suc : mb->sucs)
        if(!visited.count(suc)) {
            visited.insert(suc);
            scan_order(suc, visited);
        }
...
void linear_scan(MachineFunction *mf) {
    ...
    std::set<MachineBasicBlock*>scan_visit;
    scan_order(mf->basic_blocks[0], scan_visit);
    std::reverse(mbscan_order.begin(),mbscan_order.end());
    
    unsigned num = 0;
    std::vector<int>vid;
    std::vector<MachineOperand**>to_alloc;

    for(auto&mb: mbscan_order)
        for(auto&inst: mb->insts) {
            vid.clear();
            if(auto i = dynamic_cast<Binary*>(inst)) {
                if(auto v = dynamic_cast<VReg*>(i->dst)) vid.emplace_back(v->id);
                if(auto v = dynamic_cast<VReg*>(i->lhs)) vid.emplace_back(v->id);
                if(auto v = dynamic_cast<VReg*>(i->rhs)) vid.emplace_back(v->id);
            }
            else if(auto i = dynamic_cast<Cmp*>(inst)) {
                if(auto v = dynamic_cast<VReg*>(i->lhs)) vid.emplace_back(v->id);
                if(auto v = dynamic_cast<VReg*>(i->rhs)) vid.emplace_back(v->id);
                ...
                if(auto v = dynamic_cast<VReg*>(i->dst)) vid.emplace_back(v->id);
                if(auto v = dynamic_cast<VReg*>(i->src)) vid.emplace_back(v->id);
            } 
            else if(auto i = dynamic_cast<IClz*>(inst)) {
                if(auto v = dynamic_cast<VReg*>(i->dst)) vid.emplace_back(v->id);
                if(auto v = dynamic_cast<VReg*>(i->src)) vid.emplace_back(v->id);
                ...
                if(auto v = dynamic_cast<VReg*>(i->dst)) vid.emplace_back(v->id);
                if(auto v = dynamic_cast<VReg*>(i->base)) vid.emplace_back(v->id);
                if(auto v = dynamic_cast<VReg*>(i->offset)) vid.emplace_back(v->id);
            }
            else if(auto i = dynamic_cast<Store*>(inst)) {
                if(auto v = dynamic_cast<VReg*>(i->src)) vid.emplace_back(v->id);
                if(auto v = dynamic_cast<VReg*>(i->base)) vid.emplace_back(v->id);
    ...
    for(auto&mb: mbscan_order) {
        ...
        for(auto&inst: mb->insts) {
            to_alloc.clear();
            if(auto i = dynamic_cast<Binary*>(inst)) {
                if(auto v = dynamic_cast<VReg*>(i->dst)) to_alloc.emplace_back(&(i->dst));
                if(auto v = dynamic_cast<VReg*>(i->lhs)) to_alloc.emplace_back(&(i->lhs));
                if(auto v = dynamic_cast<VReg*>(i->rhs)) to_alloc.emplace_back(&(i->rhs));
            }
            else if(auto i = dynamic_cast<Cmp*>(inst)) {
                if(auto v = dynamic_cast<VReg*>(i->lhs)) to_alloc.emplace_back(&(i->lhs));
                if(auto v = dynamic_cast<VReg*>(i->rhs)) to_alloc.emplace_back(&(i->rhs));
                ...
                if(auto v = dynamic_cast<VReg*>(i->dst)) to_alloc.emplace_back(&(i->dst));
                if(auto v = dynamic_cast<VReg*>(i->src)) to_alloc.emplace_back(&(i->src));
            } 
            else if(auto i = dynamic_cast<IClz*>(inst)) {
                if(auto v = dynamic_cast<VReg*>(i->dst)) to_alloc.emplace_back(&(i->dst));
                if(auto v = dynamic_cast<VReg*>(i->src)) to_alloc.emplace_back(&(i->src));
                ...
                if(auto v = dynamic_cast<VReg*>(i->dst)) to_alloc.emplace_back(&(i->dst));
                if(auto v = dynamic_cast<VReg*>(i->base)) to_alloc.emplace_back(&(i->base));
                if(auto v = dynamic_cast<VReg*>(i->offset)) to_alloc.emplace_back(&(i->offset));
            }
            else if(auto i = dynamic_cast<Store*>(inst)) {
                if(auto v = dynamic_cast<VReg*>(i->src)) to_alloc.emplace_back(&(i->src));
                if(auto v = dynamic_cast<VReg*>(i->base)) to_alloc.emplace_back(&(i->base));
            ...
            for(auto operand:to_alloc){
                ...
                if(visited.count(vreg->id)) {
                    if(active_list.count(vreg->id)) *operand = active_list[vreg->id];
                        ...
                        bool isInt = vreg->operand_type == MachineOperand::OperandType::Int;
                        auto str = new Store(isInt ? Store::Tag::Int : Store::Tag::Float,
                                                   isInt ? new MReg(MReg::Reg(r)) : new MReg(MReg::Reg(s)),
                                                   new MReg(MReg::Reg::fp),
                                                   new IImm(-(local_var_size + (isInt ? MReg2Int[r - int(MReg::Reg::r0)] : MReg2Float[s - int(MReg::Reg::s0)]) * 4) - 4));
                        auto ldr = new Load(isInt ? Load::Tag::Int : Load::Tag::Float,
                                                   isInt ? new MReg(MReg::Reg(r)) : new MReg(MReg::Reg(s)),
                                                   new MReg(MReg::Reg::fp),
                                                   new IImm(-(local_var_size + vreg->id * 4) - 4));
                        mb->insts.insert(it, ldr);
                        mb->insts.insert(it, str);

                        auto it0 = it;
                        it0++;
                        ldr = new Load(isInt ? Load::Tag::Int : Load::Tag::Float,
                                       isInt ? new MReg(MReg::Reg(r)) : new MReg(MReg::Reg(s)),
                                       new MReg(MReg::Reg::fp),
                                       new IImm(-(local_var_size + (isInt ? MReg2Int[r - int(MReg::Reg::r0)] : MReg2Float[s - int(MReg::Reg::s0)]) * 4) - 4));
                        str = new Store(isInt ? Store::Tag::Int : Store::Tag::Float,
                                        isInt ? new MReg(MReg::Reg(r)) : new MReg(MReg::Reg(s)),
                                        new MReg(MReg::Reg::fp),
                                        new IImm(-(local_var_size + vreg->id * 4) - 4));
                        mb->insts.insert(it0, ldr);
                        mb->insts.insert(it0, str);
                    ...
                    if(vreg->operand_type == MachineOperand::OperandType::Int) {
                        if(intReg) {
                            for(int u = 0; u < 11; u++)
                                if((1u << u) & intReg) {
                                    intReg ^= (1u << u);
                                    *operand = new MReg(MReg::Reg(u + 1));
                                    MReg2Int[u] = vreg->id;
                                    break;
                                }
                        }
                        else {  //the core eviction algorithm comes into play
                            auto p = int_evict_candidates.top();
                            int_evict_candidates.pop();
                            auto str = new Store(Store::Tag::Int,
                                                       active_list[p.second],
                                                       new MReg(MReg::Reg::fp),
                                                       new IImm(-(local_var_size + p.second * 4) - 4));
                            mb->insts.insert(it, str);
                            *operand = active_list[vreg->id] = active_list[p.second];
                            MReg2Int[int(dynamic_cast<MReg*>(*operand)->reg) - int(MReg::Reg::r0)] = vreg->id;
                            active_list.erase(p.second);
                            int_evict_candidates.emplace(id2ranges[vreg->id], vreg->id);
                        }
                    }
                    else{
                        if(floatReg) { 
                            for(int u = 0; u < 32; u++)
                                if((1u << u) & floatReg){
                                    intReg ^= (1u << u);
                                    *operand = new MReg(MReg::Reg(17 + u));
                                    MReg2Float[u] = vreg->id;
                                    break;
                                }
                        }
                        else {
                            auto p = float_evict_candidates.top();
                            float_evict_candidates.pop();
                            ...
                            mb->insts.insert(it, str);
                            *operand = active_list[vreg->id] = active_list[p.second];
                            MReg2Float[int(dynamic_cast<MReg*>(*operand)->reg) - int(MReg::Reg::s0)] = vreg->id;
                            active_list.erase(p.second);
                            float_evict_candidates.emplace(id2ranges[vreg->id], vreg->id);
                        }
                    }
